	...
	struct scrub_ctx	*ctx)
{
	...
	verifier_threads = disk_heads(ctx->verify_disks[XFS_DEV_DATA]);
	if (verifier_threads == 1)
		verifier_threads = 0;
	ret = -workqueue_create_bound(&vc.wq_ddev, ctx, verifier_threads, 500);
	if (ret) {
		str_liberror(ctx, ret,
				_("creating data dev fsverity workqueue"));
		return ret;
	}

	/* Create thread pool for rtdev fsverity processing. */
	if (ctx->fsinfo.fs_rt) {
		verifier_threads = disk_heads(ctx->verify_disks[XFS_DEV_RT]);
		if (verifier_threads == 1)
			verifier_threads = 0;
	...
	ret = scrub_scan_all_inodes(ctx, schedule_verity_file, &vc);
	if (ret)
		goto out_rtdev;
	if (vc.aborted) {
		ret = ECANCELED;
